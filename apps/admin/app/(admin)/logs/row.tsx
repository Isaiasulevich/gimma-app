    ...
    <div className="rounded border">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between p-3 text-left hover:bg-neutral-50">
        <div className="flex flex-wrap items-center gap-3 text-sm">
          ...
          <span className="font-mono text-xs">{row.provider} / {row.model}</span>
          <span className="text-neutral-500">{row.latency_ms ? `${row.latency_ms}ms` : '—'}</span>
          <span className="text-neutral-500">in:{row.input_tokens ?? '-'} out:{row.output_tokens ?? '-'}</span>
          <span className="text-neutral-500">{row.cost_usd !== null ? `$${Number(row.cost_usd).toFixed(4)}` : '—'}</span>
        </div>
      ...
      {open && (
        <div className="border-t bg-neutral-50 p-3 font-mono text-xs">
          {row.error && <div className="mb-3 whitespace-pre-wrap text-red-700">Error: {row.error}</div>}
